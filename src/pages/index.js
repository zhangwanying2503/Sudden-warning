...
import Overall_S from './index.css'
...
import Amcharts4 from '../components/amcharts/chart4'
class Overall_situation extends Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
  render() {
    let {RealTimeData,getChartData}=this.props.Overall_situationAR
    return (
      <div className={Overall_S.con}>
        <div className={Overall_S.topbox}>
          <Numchange values={1207800} timer={3.75} suffix="Tor" />
          <Numchange values={38078} timer={2.75} suffix="I2P" />
          <Numchange values={56319} timer={3.25} suffix="ZeroNet" />
          <Numchange values={30018} timer={2.75} suffix="Telegram" />
          <Numchange values={42078} timer={2.75} suffix="whatsAPP" />
        </div>
        <div className={Overall_S.chart}>
          <div className={Overall_S.chartdivbox}>
            <p>突发事件</p>
            <div id="chart1" style={{ height: 404 }}></div>
          </div>
          <div className={Overall_S.portlet}>
          {RealTimeData.length>0 && <RealtimeData RealTimeData={RealTimeData}/>}
            
          </div>
          <div className={Overall_S.chartdivbox}>
            <p>采集量趋势</p>
            <div id="chart3" style={{ height: 404 }}></div>
          </div>
          <div className={Overall_S.chartdivbox}>
            <p>事件类型</p>
            {getChartData.length>0 && <Amcharts4 getChartData={getChartData}/>}
          </div>
        </div>
  ...
  static getDerivedStateFromProps(nextProps,prevState){
      ...
      return {getChartData:nextProps.Overall_situationAR.getChartData}
  }
  componentDidMount() {
    this.props.dispatch({
    ...
    am4core.useTheme(am4themes_animated);
    let chart3 = am4core.create("chart3", am4charts.XYChart);
